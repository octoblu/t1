/* eslint-disable no-param-reassign */

const bindAll = require('lodash/fp/bindAll')
const Motor = require('./Motor')

class Robot {
  constructor() {
    bindAll(Object.getOwnPropertyNames(Robot.prototype), this)
    this.leftMotor = new Motor({ pin: 18 })
    this.rightMotor = new Motor({ pin: 17 })
  }

  backward() {
    throw new Error('Not Implemented') // needs the sign bit
  }

  forward() {
    this.leftMotor.go(255)
    this.rightMotor.go(255)
  }

  left() {
    this.leftMotor.stutter(255, 10)
    this.rightMotor.go(255)
  }

  move({ forward, turn }) {
    forward = 2 * (forward - 128)
    this.leftMotor.go((forward - 128) + (turn - 128))
    this.rightMotor.go(forward + (128 - turn))
  }

  right() {
    this.leftMotor.go(255)
    this.rightMotor.stutter(255, 10)
  }

  stop() {
    this.leftMotor.stop()
    this.rightMotor.stop()
  }
}

module.exports = Robot
